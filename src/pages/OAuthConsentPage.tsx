...
export default function OAuthConsentPage({ user, signIn, signUp }: OAuthConsentPageProps) {
  ...
  if (!user) {
    // AuthPage keeps the OAuth query string in the browser URL, so signing in
    // returns to this exact consent request without exposing it to the app.
      ...
      <div className="w-full max-w-lg space-y-6 rounded-[1.35rem] border border-white/10 bg-black/25 p-6 text-white shadow-[0_24px_80px_rgba(0,0,0,0.5)] backdrop-blur-md">
        ...
        {details && !loading && (
          <div className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-white/50">Application</p>
              <p className="mt-1 text-lg font-semibold">{clientName}</p>
            </div>
            {details.redirect_uri && (
              <div className="flex items-start gap-2 text-sm text-white/70"><ExternalLink className="mt-0.5 h-4 w-4 shrink-0" /><span className="break-all">{details.redirect_uri}</span></div>
            )}
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-white/50">This connection can read</p>
              <ul className="mt-2 space-y-1 text-sm text-white/80">
                {(scopes.length ? scopes : ["Your read-only daily operations summary"]).map((scope) => <li key={scope}>• {scope}</li>)}
              </ul>
            </div>
        ...
        <a href={returnToConsent()} className="sr-only">Return to consent</a>
      </div>
    </div>
  );
}
